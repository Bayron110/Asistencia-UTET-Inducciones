...
function calcCarrerasMap(lista) {
  ...
  return Object.entries(map)
    .map(([nombre, d]) => ({
      nombre, total: d.total, presentes: d.presentes,
...
/* ═══════════════════════════════════════
   FEED — persistencia en localStorage
═══════════════════════════════════════ */

/** Clave única por fecha: "itsqmet_feed_2025-06-02" */
function feedKey() {
  return 'itsqmet_feed_' + new Date().toISOString().slice(0, 10);
}

/** Carga el feed guardado hoy (si existe) */
function cargarFeedLocal() {
  try {
    const raw = localStorage.getItem(feedKey());
    actividadFeed = raw ? JSON.parse(raw) : [];
  } catch {
    actividadFeed = [];
  }
}

/** Guarda el feed actual en localStorage */
function guardarFeedLocal() {
  try {
    localStorage.setItem(feedKey(), JSON.stringify(actividadFeed.slice(0, 50)));
  } catch {}
}

/** Limpia todas las claves de feed de días anteriores */
function limpiarFeedsViejos() {
  try {
    const hoy = feedKey();
    Object.keys(localStorage)
      .filter(k => k.startsWith('itsqmet_feed_') && k !== hoy)
      .forEach(k => localStorage.removeItem(k));
  } catch {}
}

/**
 * Programa un reset automático a las 00:00:00 del día siguiente.
 * Si la app queda abierta varios días, se reprograma recursivamente.
 */
function programarResetMedianoche() {
  ...
  const msRestantes = manana - ahora;
  ...
  setTimeout(() => {
    ...
    limpiarFeedsViejos();
    renderFeedActividad();
    programarResetMedianoche(); // reprogramar para el día siguiente
  }, msRestantes);
}
...
function detectarCambios(nuevos) {
  if (!Object.keys(prevEstudiantes).length) {
    // ── Primera carga: reconstruir desde timestampRegistro ──
    // Evitamos duplicar entradas que ya están en el feed cargado de localStorage
    const hoy       = new Date().toISOString().slice(0, 10);
    const yaEnFeed  = new Set(actividadFeed.map(ev => ev.cedula));

    const deHoy = Object.entries(nuevos)
      .filter(([ced, d]) => {
        if (yaEnFeed.has(ced))       return false; // ya está en el feed local
        if (!d.timestampRegistro)    return false; // no tiene timestamp
        if (d.asistencia !== true)   return false; // solo presentes
        const fechaReg = new Date(d.timestampRegistro).toISOString().slice(0, 10);
        return fechaReg === hoy;
      })
      .sort(([, a], [, b]) => b.timestampRegistro - a.timestampRegistro);
    ...
    deHoy.forEach(([ced, d]) => {
      actividadFeed.push({
        ...
        asistencia: true,
        ts:         d.timestampRegistro
      });
    });

    // Reordenar por timestamp descendente después de mezclar
    actividadFeed.sort((a, b) => b.ts - a.ts);

    if (deHoy.length) guardarFeedLocal();
    prevEstudiantes = { ...nuevos };
    return;
  }

  // ── Cambios en tiempo real (app abierta) ──
  ...
  if (actividadFeed.length > 50) actividadFeed = actividadFeed.slice(0, 500);
  guardarFeedLocal();
  prevEstudiantes = { ...nuevos };
}
...
function renderTablaViva() {
  ...
  if (q) entries = entries.filter(([ced, v]) =>
    ...
    (v.nombres || v.nombre || '').toLowerCase().includes(q) ||
    (v.carrera || '').toLowerCase().includes(q));
  ...
  chartInstance = new Chart(cvs, {
    ...
    options: {
      ...
      scales: {
        x: {
          min: 0, max: 100,
          grid: { color: 'rgba(255,255,255,.05)' }, border: { color: 'rgba(255,255,255,.07)' },
...
function montarHTML() {
  ...
  tab.innerHTML = `
    ...
    <style>
      ...
      .dash-donut-card { padding: 20px !important; }
      .dash-donut-card__title { margin-bottom: 14px !important; }
      ...
      .dash-ministats-title {
        ...
        text-transform: uppercase; color: #4a607a;
      }
      .dms-meta-wrap { display:flex; flex-direction:column; gap:6px; }
      .dms-meta-label { display:flex; justify-content:space-between; font-size:.78rem; color:#7a8fa8; }
      .dms-meta-track {
        position: relative; height: 8px;
        background: rgba(255,255,255,.07); border-radius: 99px; overflow: visible;
      }
      .dms-meta-fill { height:100%; border-radius:99px; transition:width .6s cubic-bezier(.4,0,.2,1),background .4s; }
      .dms-row {
      ...
      .dms-row-icon { width:34px; height:34px; border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
      .dms-row-body { display:flex; flex-direction:column; gap:3px; min-width:0; flex:1; }
      .dms-row-lbl { font-size:.67rem; color:#4a607a; text-transform:uppercase; letter-spacing:.06em; font-weight:700; }
      .dms-row-val { font-size:.82rem; color:#c4d4e8; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      .dms-row-pct { font-family:'JetBrains Mono',monospace; font-size:1.05rem; font-weight:700; flex-shrink:0; }
      ...
      .dash-feed-title { font-size:.8rem; font-weight:700; color:#c4d4e8; }
      .dash-feed-badge {
        background: rgba(96,165,250,.15); border: 1px solid rgba(96,165,250,.3);
        color: #60a5fa; font-size:.68rem; font-family:'JetBrains Mono',monospace;
        font-weight:700; padding:3px 10px; border-radius:99px; min-width:28px; text-align:center;
      }
      .daf-list { list-style:none; margin:0; padding:6px 0; overflow-y:auto; max-height:280px; }
      ...
      .daf-empty {
        ...
        padding:28px 16px; color:#4a607a; text-align:center; font-size:.78rem; line-height:1.6;
      }
      .daf-item { display:flex; gap:0; padding:0; animation:daf-in .3s ease both; }
      @keyframes daf-in { from{opacity:0;transform:translateY(-6px)} to{opacity:1;transform:translateY(0)} }
      .daf-dot-wrap { display:flex; flex-direction:column; align-items:center; padding:12px 0 0 16px; width:40px; flex-shrink:0; }
      .daf-dot { width:24px; height:24px; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0; color:#fff; }
      .daf-line { width:1px; flex:1; min-height:10px; background:rgba(255,255,255,.07); margin-top:5px; }
      .daf-item:last-child .daf-line { display:none; }
      .daf-body { padding:10px 16px 10px 10px; flex:1; min-width:0; border-bottom:1px solid rgba(255,255,255,.04); }
      .daf-item:last-child .daf-body { border-bottom:none; }
      .daf-row1 { display:flex; justify-content:space-between; align-items:baseline; gap:8px; margin-bottom:3px; }
      .daf-nombre { font-size:.8rem; font-weight:600; color:#c4d4e8; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      .daf-ts { font-size:.64rem; color:#4a607a; font-family:'JetBrains Mono',monospace; flex-shrink:0; }
      .daf-row2 { display:flex; justify-content:space-between; align-items:center; gap:8px; }
      .daf-carrera { font-size:.7rem; color:#4a607a; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      .daf-estado { font-size:.7rem; font-weight:700; flex-shrink:0; }

      /* Responsive */
      @media (max-width:1100px) { .dash-main-row { grid-template-columns:300px 1fr !important; } }
      @media (max-width:900px) {
        .dash-main-row { grid-template-columns:1fr !important; }
        .dash-left-col { display:grid !important; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:14px; }
      }
      @media (max-width:640px) {
        ...
        .dash-left-col { display:flex !important; flex-direction:column !important; }
        .dash-donut-card { padding:16px !important; }
        .dash-table-head { flex-direction:column !important; align-items:flex-start !important; gap:10px !important; }
        .dash-search-wrap { width:100% !important; }
        #dashSearch { width:100% !important; }
        .dash-section-hd { flex-direction:column !important; align-items:flex-start !important; gap:10px !important; }
        .dash-section-filters { flex-wrap:wrap !important; gap:6px !important; }
        .dv-leyenda { display:none !important; }
        .dgh-card { border-radius:12px !important; }
        .dgh-stats { flex-wrap:wrap !important; gap:6px !important; }
        .daf-list { max-height:220px !important; }
      }
      @media (max-width:400px) {
        .dash-kpi-row { grid-template-columns:1fr 1fr !important; gap:8px !important; }
        .dash-kpi__num { font-size:1.6rem !important; }
        .dash-filter-row { flex-wrap:wrap !important; gap:6px !important; }
        .dv-chips { gap:5px !important; }
        .dv-chip { font-size:.7rem !important; padding:5px 9px !important; }
      }
    </style>
  `;
...
export function iniciarDashboard() {
  ...
  // 1. Cargar feed persistido del día actual
  cargarFeedLocal();

  // 2. Timer de reset automático a medianoche
  programarResetMedianoche();

  // 3. Listener de estudiantes
  if (!listenerIniciado) {
