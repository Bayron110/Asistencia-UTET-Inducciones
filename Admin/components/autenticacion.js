...
async function verificarEstadoAdmin() {
  try {
    ...
    if (adminRegistrado) {
      ...
      if (registerHint) {
        registerHint.textContent = 'Ingresa con la contraseña del administrador.';
      }
...
function bindEventosAutenticacion() {
  ...
  if (registerAdminBtn) {
    registerAdminBtn.addEventListener('click', registrarPrimerAdmin);
  }

  if (logoutBtn) {
    logoutBtn.addEventListener('click', cerrarDashboard);
  }
